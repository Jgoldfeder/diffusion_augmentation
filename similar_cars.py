...
def get_command_string(command,gpu,aug_directory,data_dir):
    def get_command(device,aug_directory,model,name,experiment,recipe,shots,variations,ways,repeats,seed,data_directory):
        ...
        return 'CUDA_VISIBLE_DEVICES='+str(device)+' python3 train.py '+data_directory+' --dataset hfds/'+dataset+'  --model='+model+'  --num-classes=397  --log-wandb --experiment "'+experiment+'" --diffaug-dir='+aug_directory+' --seed '+str(seed)+'  --name "'+name+'" --recipe "'+recipe+'"  --repeats '+str(repeats)+' --variations '+str(variations)+' --diffaug-fewshot='+str(shots)+' '+ways+' '
...
def get_fewshot_commands_cars_pretrain_sunswitch():
    commands = []
        ...
        for seed in seeds:
            for way in ways:
                if way=="all":
                    way_str = ""
                if way=="5":
                    way_str = " --classes 25 27 28 32 34 "
                if way=="10":
                    way_str= " --classes  10 23 110 12 40 41 100 96 95 94 "
                for shot in shots:
                    for variation in variations:            
                        experiment = dataset + "-" + way + "-" + str(shot) + "-" + str(variation) + "-similar_cars"
...
commands = get_fewshot_commands_cars_pretrain_sunswitch()
