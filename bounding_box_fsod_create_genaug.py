...
# Import tree augmentation components
...
def apply_classical_augmentation_with_tracking(img, transform):
    """
...
def create_augmentation_tree_without_classical(num_levels):
    """
    Create a random augmentation tree that excludes CLASSICAL and NONE augmentations.
    ...
    def make_tree_recursive(levels_remaining):
        ...
        # Get a random augmentation type excluding CLASSICAL and NONE
        valid_types = [t for t in AugmentationType if t not in 
                      (AugmentationType.CLASSICAL, AugmentationType.NONE)]
        
        if not valid_types:
            raise ValueError("No valid augmentation types available")
...
def augment_dataset_with_tree(
    subset_path, 
    base_output_root, 
    num_augmentations=2, 
    tree_levels=2,
    output_suffix="tree_classical"
):
    """
    Create augmented version of dataset using tree augmentation + classical.
    ...
    Args:
        ...
        tree_levels: depth of augmentation tree
        output_suffix: suffix for output directory (e.g., "tree_classical")
    """
    # Original dataset path
    original_root = os.path.join(base_output_root, subset_path)
    
    # New augmented dataset path
    augmented_root = os.path.join(base_output_root, output_suffix, subset_path)
    
    print(f"Original dataset: {original_root}")
    print(f"Augmented dataset: {augmented_root}")
    
    # Create augmentation tree (without CLASSICAL nodes)
    print(f"Creating augmentation tree with {tree_levels} levels (excluding CLASSICAL)...")
    augmentation_tree = create_augmentation_tree_without_classical(tree_levels)
    ...
    # Get classical augmentation transform
    ...
    print(f"Processing {len(train_coco['images'])} training images with tree + classical augmentation...")
    ...
    for img_info in train_coco["images"]:
        ...
        # Get a representative category name (use first annotation's category)
        class_name = "unknown"
        if img_annotations:
            cat_id = img_annotations[0]["category_id"]
            class_name = cat_id_to_name.get(cat_id, "unknown")
        
        # Create N augmented versions
        for aug_idx in range(num_augmentations):
            # Step 1: Apply tree augmentation (doesn't affect bounding boxes)
            tree_augmented_img = augmentation_tree.generate_augmentation(img, class_name)
            
            # Step 2: Apply classical augmentation with tracking
            final_img, transform_params = apply_classical_augmentation_with_tracking(
                tree_augmented_img, classical_transform
            )
            ...
            final_img.save(aug_img_path)
            
            # Transform and add annotations (only classical transforms affect bbox)
            ...
            visualize_annotations(final_img, transformed_annotations, categories_dict, vis_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply tree + classical augmentations to FSOD few-shot dataset."
    )
    parser.add_argument(
        "--subset-path",
        type=str,
    ...
    parser.add_argument(
        "--output-suffix",
        type=str,
        default="tree_classical",
        help="Suffix for output directory (e.g., 'tree_classical' creates tree_classical/subset_0).",
    )
    args = parser.parse_args()
    
    augment_dataset_with_tree(
