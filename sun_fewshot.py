...
class Machine0:
    def __init__(self):
        self.name = "machine_0"
        self.aug_dir = "/data/puma_envs/control_augmented_images_sun397_512fewshot"
        self.data_dir = "/data/torch/sun397"
...
def get_fewshot_commands_sun_pretrain_switch():
    commands = []
        ...
        for seed in seeds:
            for way in ways:
                if way=="all":
                    way_str = ""
                if way=="5":
                    way_str = " --classes 191 222 172 194 8 "  #0 82 85 95 231 "
                if way=="10":
                    way_str= " --classes 191 222 172 194 8 0 82 85 95 242"  #0 82 85 95 231 240 242 343 191 222 "
                for shot in shots:
                    for variation in variations:            
                        experiment = dataset + "-" + way + "-" + str(shot) + "-" + str(variation) + "-pretrain_finegrain2s"
...
commands = get_fewshot_commands_sun_pretrain_switch()
